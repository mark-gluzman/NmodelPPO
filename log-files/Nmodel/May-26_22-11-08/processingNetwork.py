...
class ProcessingNetwork:
    ...
    def next_state_prob(self, states_array):
        ...
        for action_ind, action in enumerate(self.actions):
            ...
            possible_activities = 1*(np.dot(states , constr_trans) > 0)
            for arv_i in range(arrivals_num):
                possible_activities[:, arv_i] = 1

            # probability for each activity, except fake one
            prob = np.dot( possible_activities , np.diag(np.hstack([self.p_arriving[self.p_arriving > 0], self.p_compl[self.p_compl > 0]])))
    ...
    def random_policy_distr(self, state):
        """
        Return probability distribution of actions for each station based on Random proportional policy
        :param state: system state
        :return: distribution of action according to random proportional policy
        ...
        for server in range(self.stations_num):
            distr_one_server = np.zeros((1, np.sum(self.D[server])))
            z_sum = np.sum(state[self.D[server]>0]>0) # sum of jobs that wait to be processed in 'server'
            if z_sum > 0:
                all_states = (state>0) / z_sum
                distr_one_server = np.reshape(all_states[self.D[server]>0], (1, np.sum(self.D[server])))
            else:
                distr_one_server[0]  = 1./sum(self.D[server])
            distr.append(distr_one_server)
    ...
    def next_state_N1(self, state, action):
        ...
        alpha = np.asarray([1.3*0.95, 0.4*0.95])
        mu = np.asarray([1, 1/2, 1])
        uniform_rate = np.sum(alpha) + np.sum(mu)
        p_arriving = np.divide(alpha, uniform_rate)
        p_compl = np.divide(mu, uniform_rate)
        cum_sum = np.unique(np.cumsum(np.hstack([p_arriving, p_compl])))
    ...
    def next_state_probN(self, states_array):
        ...
        prob = np.asarray([p_arriving[0]* np.ones(num), p_arriving[1]* np.ones(num), p_compl[0]*(states_array[:,0]>0) +
                           p_compl[1]*(states_array[:,0]>1), p_compl[2]* (states_array[:,0]<2) *  (states_array[:,1]>0)   ])
        ...
        prob2 = np.asarray([p_arriving[0]* np.ones(num), p_arriving[1]* np.ones(num), p_compl[0]*(states_array[:,0]>0) +
                           p_compl[1]*( 1*(states_array[:,0]>1) * 1*(states_array[:,1]==0) ), p_compl[2]*  (states_array[:,1]>0)  ])
